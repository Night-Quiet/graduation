...
public class CommentController {
    ...
    @RequestMapping(value = "/addComment.hml",method= RequestMethod.POST)
    public String addComment(String comment, HttpSession session) {
        ...
        boolean updateJudge = commentService.addComment(questionId, identify, userId, userName, comment);

        List<Map<String, Object>> comment_show = commentService.getCommentById(questionId);
        for (int i=0; i<comment_show.size(); i++) {
            Long id_comment = Long.valueOf(comment_show.get(i).get("id_comment").toString());
        ...
        statisticsService.updateIsComment(userId, questionId, 1);
    ...
    @RequestMapping(value = "/deleteComment.hml",method = RequestMethod.GET)
    public String deleteComment(Long id_comment, HttpSession session) {
        boolean judge = commentService.updateComment(id_comment, "评论与问题无关, 已被删除");
